<div align="center">

# 📖 扫书宝典

<i>💣 网文扫雷专家</i>

![License](https://img.shields.io/badge/license-AGPL--3.0-green?style=flat-square)
![Python](https://img.shields.io/badge/python-3.10+-blue?style=flat-square&logo=python&logoColor=white)
![AstrBot](https://img.shields.io/badge/framework-AstrBot-ff6b6b?style=flat-square)

</div>

## ✨ 简介

一款为 [**AstrBot**](https://github.com/AstrBotDevs/AstrBot) 设计的网文扫书记录查询插件。通过集成海量扫书数据库，提供网文评价、避雷说明、术语查询等功能，助你快速甄别作品质量，远离郁闷与雷点。

---

## 🚀 功能特性

* **海量记录检索**: 支持通过书名、作者名或书评全文关键词检索扫书记录，默认搭载数万条真实扫书评价。
* **详细属性展示**: 包含作品类型、评价等级、避雷说明、可能的雷点/郁闷点等丰富维度。
* **术语百科查询**: 支持查询网文圈常用术语、避雷术语、防御术语等。
* **多库查询优先**: 支持上传自定义数据库参与查询，上传顺序越靠前优先级越高。
* **智能发送策略**: QQ 自动合并转发，TG 平台自动折叠内容；超长记录可截断或分段。

---

## 💿 安装

1. **下载本仓库**。
2. 将整个 `astrbot_plugin_webnovel_bible` 文件夹放入 `astrbot` 的 `plugins` 目录中。
3. **安装依赖库**：
   ```bash
   pip install -r requirements.txt
   ```
4. 重启 AstrBot。

> [!TIP]
> 插件启动后会自动从项目 CDN 获取 manifest 并下载默认数据库，后续有更新也会同步自动更新。

---

## ⚙️ 配置说明

首次加载后，请在 AstrBot 后台 -> 插件 页面找到本插件进行设置。

| 配置项 | 类型 | 默认值 | 说明 |
| :--- | :--- | :--- | :--- |
| `max_review_length` | `int` | `2000` | 单条记录最大长度。 |
| `overflow_strategy` | `string` | `truncate` | 单记录超长处理策略，可选 `truncate` / `split`。 |
| `max_batch_chars` | `int` | `2000` | 单次合并转发总字符上限。超过将自动切分批次。 |
| `max_messages_per_request` | `int` | `3` | 单次查询最多发送的消息数量。 |
| `uploaded_db_files` | `file` | `[]` | 上传 .db 文件参与查询，上传顺序越靠前优先级越高。 |

---

## 💡 使用方法

### 1. 扫书检索

* **按书名/作者搜索**: `/扫书 <书名/作者>` 。
* **全文搜索**: `/搜扫书 <关键词>` 或 `/搜书评 <关键词>`。在扫书记录的全文内容中搜索包含关键词的书籍，结果附带匹配原文节选。
* **随机一本**: `/随机扫书`。随机从数据库中选取一本书展示其扫书记录。
* **查看详情**:
  * 搜索出结果列表后，发送 `/扫书 <序号>` 查看对应书籍的所有评价。
  * **快捷指令**: 直接发送 `/扫书 <书名> <序号>`（例如 `/扫书 极品家丁 1`）可直接跳转至第一项结果的详情。
  * **继续获取**: 发送 `/扫书 n` 获取下一批详情。

### 2. 术语查询

支持查询网文圈常用术语、避雷术语、防御术语等。

* **分类查询**:
  * `/术语 <术语>`：查询通用术语。
  * `/防御 <术语>`：查询防御相关术语。
  * `/郁闷 <术语>`：查询郁闷点术语。
  * `/雷点 <术语>`：查询雷点相关术语。
* **查看列表**: 发送对应指令加 `列表`（如 `/术语 列表`）可以查看该分类下的所有词条。

### 3. 数据统计

* **查看库情**: `/扫书统计`。实时查看当前数据库中收录的小说总数及扫书记录总条数。
> 数据库收录截止时间：2025 年 1 月

---

## 🙏 致谢

本插件的数据来源于广大书友，特别感谢：

* **各位辛勤贡献的扫书作者**
* **扫书宝典 App 原作者**

---

## 📝 更新日志

详见 [CHANGELOG.md](./CHANGELOG.md)。

---

## 💬 交流

欢迎加入网文交流群 [**1062663795**](https://qm.qq.com/cgi-bin/qm/qr?k=zU2UPWyrflqs1sCHufgVRvsejdLSglyP&jump_from=webapi&authKey=Z2Zg79IhS9KXEotmtRbtbabeuZZ0LpztVM+p+V1HyWn9K1JpuaFQr9acnmKK8G5K)，一起推书、吹水~

---

## ❤️ 支持

* [AstrBot 帮助文档](https://astrbot.app)
* 如果您在使用中遇到错误或有改进建议，例如扫书中部分条目的书名、作者名收录有误，请通过 [Issue](https://github.com/Foolllll-J/astrbot_plugin_webnovel_bible) 提出，交流群不受理插件问题。

---

<div align="center">

**如果本插件对你有帮助，欢迎点个 ⭐ Star 支持一下！**

</div>
